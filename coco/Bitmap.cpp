...
int Bitmap::drawText(int x, int y, const LinearFont &font, String text, DrawMode mode) {
    for (auto glyph : font.glyphRange(text)) {
        if (glyph.size.y == 0) {
            // non-printable character (e.g. ' ')
            x += glyph.size.x + font.gapWidth;
        } else {
            // draw character
            copyBitmapH(x, y + glyph.y, glyph.size.x, glyph.size.y, font.data + glyph.location, mode);
