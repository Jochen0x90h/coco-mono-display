...
/// @brief Bitmap with line and text drawing functions
///
class Bitmap {
public:
    ...
    int drawText(int x, int y, const LinearFont &font, String text, DrawMode mode = DrawMode::SET);
    ...
    /// @brief Bitmap data, data layout: rows of 8 pixels where each byte describes a column in each row
...
public:
    ...
    BitmapStream(const Bitmap &bitmap, const LinearFont &font, int x, int y) : bitmap(bitmap), font(font), x(x), y(y) {}
    ...
    BitmapStream &operator <<(const String &str) {//override {
